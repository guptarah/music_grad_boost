...
def get_top_features(residual,train_features):
	num_feats = train_features.shape[1]
	matrix_to_consider = numpy.concatenate((residual,train_features),axis=1)
	covariances = numpy.abs(numpy.corrcoef(matrix_to_consider.T)[0,:])[1:,]
	print covariances.shape
	percentile_cov = numpy.percentile(covariances,95)
	print percentile_cov
	chosen_feats = numpy.nonzero(covariances > percentile_cov)
	chosen_feats = chosen_feats[0]
	print 'num chosen feats:',chosen_feats.shape
	return chosen_feats
...
for classf_id in range(0,num_classifiers):
	...
	feat_list = get_top_features(residual,train_features)
	num_chosen_feats = feat_list.shape[0]
	dev_errors = numpy.zeros(num_chosen_feats)

	for cur_feat_ind in range(0,num_chosen_feats):
		cur_feat = feat_list[cur_feat_ind]	
		[cur_coeffs,feat_ind,classf_output] = compute_filter_given_feat(residual,train_features,train_targets,cur_feat,filt_len)
	...
	chosen_feat = feat_list[chosen_feat_ind]	

	print 'chosen feat: ', chosen_feat ,chosen_feat_ind

	[cur_coeffs,feat_ind,classf_output] = compute_filter_given_feat(residual,train_features,train_targets,chosen_feat,filt_len)
	all_coeffs[classf_id,:] = cur_coeffs
	feat_inds[classf_id,0] = chosen_feat 
