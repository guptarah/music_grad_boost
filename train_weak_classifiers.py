...
def compute_error (features,targets,feat_inds,all_coeffs,classf_id):
	...
	for k in range(0,classf_id+1):
		...
		for i in range(0,num_train_files):
			...
			print cur_file_feat_vals.shape
        	        # making matrix
                	for j in range(0,60):
                        	if j < filt_len:
...
#def compute_filter_rand_feat(residual,train_features,train_targets,filt_len):
#	num_feats = train_features.shape[1]
#	cur_feat_ind = numpy.random.randint(num_feats)
...
#	num_feat_wins = num_train_files * (60 - 2*filt_len)	
#	feat_mat = numpy.zeros((num_feat_wins,feat_win_len))
...
#		# making matrix
#		for j in range(filt_len,60-filt_len): # not considering the values in begining and end
#			target[i*(60 - 2*filt_len) + j - filt_len] = cur_file_residual[j]
#			cur_feat_window = cur_file_feat_vals[j-filt_len:j+filt_len+1]
#			feat_mat[i*(60 - 2*filt_len) + j - filt_len,:] = cur_feat_window			
#					
...
for classf_id in range(0,num_classifiers):
	...
	[cur_coeffs,feat_ind,classf_output] = compute_filter_rand_feat(residual,train_features,train_targets,filt_len)
	...
	feat_inds[classf_id,0] = feat_ind
	...
	residual = residual - gamma*classf_output
